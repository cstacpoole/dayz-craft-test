class RepairedSparkPlug extends RecipeBase   // Our example recipe which uses primary ingredient - our custom item MagicHammer to turn any other secondary ingredient ( all ItemBase items ) into stones. 
{
	override void Init()
	{
		m_Name = "Repair Spark Plug";	// action name in game
		...
		m_MinDamageIngredient[1] = 4;	//4 - Ruined - plug must be ruined
		...
		//ingredient 2	
		InsertIngredient(1,"SparkPlug"); //  secondary ingredient
		...
		//result
		AddResult("RepairedSparkPlug");	// recipe result
		
		m_ResultSetFullQuantity[0] = -1;	// -1 = do nothing
		m_ResultSetQuantity[0] = 4;			// result quantity
		m_ResultSetHealth[0] = 5;			// Set Health to 10 - Badly Damaged
